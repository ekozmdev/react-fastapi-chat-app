...
class ToolExecutionManager:
    ...
    def record_tool_call(self, tool_call_id: str, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.info(f"Tool call recorded: {tool_name} (order: {self.execution_order})")
    ...
    def start_execution(self, tool_call_id: str) -> bool:
        ...
        try:
            ...
            if execution:
                ...
                # Phase 1互換性の更新
                if self.current_tool and self.current_tool.get("execution_id") == execution.id:
        ...
        try:
            ...
            if execution:
                ...
                if execution.started_at:
                    execution_time = int((datetime.now(UTC) - execution.started_at).total_seconds() * 1000)
                else:
                    execution_time = int((datetime.now(UTC) - execution.called_at).total_seconds() * 1000)
                ...
                logger.info(f"Tool execution completed: {execution.tool_name} ({execution_time}ms)")
                return True
    ...
    def _get_execution_by_call_id(self, tool_call_id: str) -> Optional[ToolExecution]:
        """ツールコールIDから実行記録を取得"""
        try:
    ...
    def get_all_executions(self) -> List[ToolExecution]:
        ...
        try:
            return self.db.query(ToolExecution).filter(
                ToolExecution.message_id == self.message_id
            ).order_by(ToolExecution.execution_order).all()
        except Exception as e:
