...
class EnhancedToolStreamHandler:
    ...
    async def _notify_tool_start(
        self, tool_name: str, tool_call_id: str, arguments: dict[str, Any] = None
    ):
        """ツール開始の通知"""
    ...
    async def enhanced_stream_events(
        self, agent: Agent, messages: list[dict[str, Any]]
            ...
            if hasattr(event, "data") and event.data is not None:
                ...
                for tool_call in tool_calls:
                    ...
                    if tool_call_id not in self.pending_tools:
                        ...
                        # 早期検出の通知
                        await self._notify_tool_start(
                            tool_name, tool_call_id, arguments
                        )
                        logger.info(
                            f"Early tool detection: {tool_name} ({tool_call_id})"
                        )
        ...
        try:
            if event.item.type == "tool_call_item":
                ...
                tool_call_id = getattr(
                    raw_item, "call_id", f"{tool_name}_{int(time.time() * 1000)}"
                )
                ...
                raw_item = event.item.raw_item
                tool_call_id = (
                    raw_item.get("call_id", "unknown")
                    if isinstance(raw_item, dict)
                    else getattr(raw_item, "call_id", "unknown")
                )
                ...
                if tool_call_id in self.pending_tools:
                    ...
                    logger.info(
                        f"Tool completed: {tool_info['name']} ({execution_time:.2f}s)"
                    )
    ...
    def _extract_tool_calls_from_raw_event(self, event) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if (
                hasattr(data, "delta")
                and data.delta
                and hasattr(data.delta, "tool_calls")
            ):
        ...
        return {
            ...
            "success_count": len(
                [
                    ...
                    if "output" in t and not t.get("error")
                ]
            ),
...
# 使用例とヘルパー関数
async def create_enhanced_sse_stream(
    agent: Agent, messages: list[dict[str, Any]], tool_callback=None
):
    """拡張SSEストリームの作成"""
